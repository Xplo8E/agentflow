...
That wrapper now runs both the raw shell-level Kimi toolchain check and the bundled `agentflow toolchain-local`, `inspect`, `doctor`, `smoke`, and `check-local` commands before it moves on to the external custom-pipeline `doctor`, `inspect`, `check-local`, and `run` paths for `bootstrap: kimi`, explicit `shell_init: kimi`, and explicit `target.shell` Kimi wrappers. This keeps the maintainer path closer to the real user-facing debugging flow, so regressions in the bundled example run, inspect, or doctor surfaces do not hide behind a still-green custom pipeline. It also prints whether `~/.bash_profile`, `~/.bash_login`, or `~/.profile` is supplying the bash login startup path before it verifies `kimi`, `codex`, and `claude`, reports whether Codex auth is coming from `OPENAI_API_KEY`, Codex CLI login, or both in that shared Kimi-backed shell, and shows the resolved `codex` / `claude` executable paths so it is easier to catch mixed Node/npm installs.
